...
Utiliza-se o conjunto de dados \textit{Air Quality} do repositório UCI \textit{Machine Learning Repository} \cite{airqualityuci2008}, composto por medições horárias realizadas entre março de 2004 e abril de 2005 em um ambiente urbano ao nível da rua. O conjunto integra medições de referência de poluentes, respostas de uma matriz de sensores químicos de óxido metálico (MOx) e variáveis meteorológicas.

As variáveis do conjunto de dados foram agrupadas conforme sua natureza e papel no estudo. A Tabela~\ref{tab:dataset-simple} apresenta uma visão sintética das variáveis utilizadas.
\begin{table}[htbp]
\centering
\caption{Resumo das variáveis do conjunto de dados \textit{Air Quality}.}
\label{tab:dataset-simple}
\begin{tabular}{l l l}
\toprule
\textbf{Grupo} & \textbf{Variáveis} & \textbf{Uso no estudo} \\
\midrule
Poluentes & CO, C$_6$H$_6$, NO$_x$, NO$_2$ & Índice de poluição \\
Sensores MOx & PT08.S1 -- PT08.S5 & Preditores \\
Ambientais & T, RH, AH & Preditores \\
\bottomrule
\end{tabular}
\end{table}
...
O pré-processamento teve como objetivo viabilizar a aplicação dos algoritmos de classificação e assegurar consistência estatística dos dados. Os dados foram carregados respeitando o separador e a codificação decimal originais, e os valores ausentes identificados pelo código \texttt{-200} foram convertidos para valores nulos. Colunas sem informação relevante e registros incompletos foram removidos.

A variável \texttt{NMHC(GT)} foi excluída devido à elevada proporção de valores ausentes (superior a 90\%). Em seguida, o conjunto de dados foi particionado de forma estratificada em conjuntos de treino (70\%) e teste (30\%), preservando a distribuição das classes do índice de poluição.

As variáveis preditoras foram normalizadas por meio da padronização \textit{z-score}, definida por:
\begin{equation}
z_i = \frac{x_i - \mu}{\sigma},
\end{equation}
em que $x_i$ representa o valor original da variável, $\mu$ a média e $\sigma$ o desvio padrão, calculados exclusivamente no conjunto de treino.
...
% \end{figure}
Para a formulação do problema de classificação, o índice contínuo foi discretizado em classes ordinais por meio de quantis empíricos, definidos a partir da distribuição do índice no conjunto de dados:
\begin{equation}
\text{Classe} =
\begin{cases}
...
A Tabela~\ref{tab:caracterizacao} apresenta a distribuição percentual das classes resultantes do procedimento de discretização.
